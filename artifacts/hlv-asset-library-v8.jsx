...
const GoodProblemCriteria = () => {
  const criteria = [
    ...
    { num: 2, title: 'Specific', desc: 'Clear who has it, where, and when — not vague or too broad', check: 'Can I name actual people with this problem?', highlight: false },
    { num: 3, title: 'Deeply Felt', desc: 'It causes real frustration, pain, or cost — not just minor annoyance', check: 'Would someone pay or change behavior to solve this?', highlight: true },
...
const ProblemOpportunityFlow = () => {
  ...
  return (
    <svg viewBox="0 0 900 420" style={{ width: '100%', maxWidth: 900 }}>
      ...
      <text x={450} y={35} textAnchor="middle" style={S.title}>Problem → Opportunity Flow</text>
...
const SymptomVsRootCause = () => {
  ...
  return (
    <svg viewBox="0 0 800 500" style={{ width: '100%', maxWidth: 800 }}>
      ...
      <text x={400} y={58} textAnchor="middle" style={S.subtitle}>Why reframing matters — treating symptoms doesn't solve problems</text>
      ...
      <g transform="translate(40, 110)">
        ...
        <text x={80} y={25} textAnchor="middle" style={{ fontFamily: T.font, fontSize: 11, fontWeight: 600, fill: '#DC2626' }}>✗ Treating Symptoms</text>
      ...
      <g transform="translate(600, 300)">
        ...
        <text x={80} y={25} textAnchor="middle" style={{ fontFamily: T.font, fontSize: 11, fontWeight: 600, fill: T.success }}>✓ Solving Root Cause</text>
      ...
      <text x={415} y={442} style={S.label}>→</text>
      <text x={435} y={442} style={{ fontFamily: T.font, fontSize: 11, fontStyle: 'italic', fill: T.slate }}>Root Cause: "People hate waiting"</text>
      <text x={80} y={462} style={{ fontFamily: T.font, fontSize: 11, fontWeight: 600, fill: '#DC2626' }}>✗ Fix: Faster elevator ($$$)</text>
      <text x={435} y={462} style={{ fontFamily: T.font, fontSize: 11, fontWeight: 600, fill: T.success }}>✓ Fix: Add mirrors (cheap, works)</text>
...
const GoodVsWeakProblems = () => {
  const comparisons = [
    { weak: { tag: 'GENERIC', problem: '"People waste food."', note: 'No personal connection — why do YOU care?' },
      good: { tag: 'PERSONAL', problem: '"My family throws away $50 of groceries weekly."', note: 'Connected to your life — motivation to persist' } },
    ...
    { weak: { tag: 'MILD', problem: '"It\'s annoying when my phone dies."', note: 'Minor inconvenience — won\'t pay to solve' },
      good: { tag: 'DEEPLY FELT', problem: '"Nurses miss family emergencies because', problem2: 'hospital policy blocks personal phones."' } },
    { weak: { tag: 'RARE', problem: '"I lost my luggage on a flight once."', note: 'Happened once — not worth building for' },
      good: { tag: 'FREQUENT', problem: '"Students forget homework assignments', problem2: '3-4 times per week across multiple classes."' } },
    { weak: { tag: 'SOLVED', problem: '"I need a way to order food online."', note: 'DoorDash, UberEats, etc. — already done well' },
      good: { tag: 'UNSOLVED', problem: '"Elderly patients can\'t track multiple', problem2: 'medications — existing apps are too complex."' } },
  ];
  ...
  return (
    <svg viewBox="0 0 900 600" style={{ width: '100%', maxWidth: 900 }}>
      ...
      <text x={450} y={58} textAnchor="middle" style={S.subtitle}>See the 5 criteria in action — before and after</text>
      
      <g transform="translate(60, 85)">
        <rect x={0} y={0} width={350} height={36} rx={6} fill="#FEE2E2" />
        <text x={175} y={24} textAnchor="middle" style={{ fontFamily: T.font, fontSize: 12, fontWeight: 500, fill: '#DC2626' }}>✗ WEAK PROBLEMS</text>
      </g>
      <g transform="translate(490, 85)">
        <rect x={0} y={0} width={350} height={36} rx={6} fill="#D1FAE5" />
        <text x={175} y={24} textAnchor="middle" style={{ fontFamily: T.font, fontSize: 12, fontWeight: 500, fill: T.success }}>✓ GOOD PROBLEMS</text>
      </g>
...
const DIAGRAMS = {
  ...
  'problem-opportunity-flow': { name: 'Problem → Opportunity Flow', usage: 'Shows the 4-stage transformation from raw problem to HMW statement', viewBox: '0 0 900 420' },
  ...
  return (
    <div style={{ fontFamily: T.font, background: '#FAFAFA', minHeight: '100vh', padding: 20 }}>
      <div style={{ borderBottom: `2px solid ${T.navy}`, paddingBottom: 12, marginBottom: 20 }}>
        ...
        <p style={{ fontSize: 12, color: T.gray, margin: '4px 0 0' }}>Design Tokens + Diagrams â€¢ v8.0</p>
      </div>
